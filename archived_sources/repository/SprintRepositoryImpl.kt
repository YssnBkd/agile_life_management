...
class SprintRepositoryImpl @Inject constructor(
    ...
    override fun getSprintsInRange(startDate: LocalDate, endDate: LocalDate): Flow<List<Sprint>> {
        // First trigger a background sync
    ...
    private suspend fun syncSprintsInRange(startDate: LocalDate, endDate: LocalDate) {
        withContext(ioDispatcher) {
                try {
    ...
    override suspend fun createSprintReview(sprintReview: SprintReview): Result<SprintReview> {
        return Result.success(sprintReview.copy(id = java.util.UUID.randomUUID().toString()))
    }
